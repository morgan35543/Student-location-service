...
<div class="content">
    ...
    <?php
        require "Config.php";
        $userquery = "SELECT [userID], CONCAT([Forename],' ',[Surname]) AS NAME FROM users ORDER BY [Forename] ASC";
        $userresult = sqlsrv_query($conn,$userquery); 
        $locationquery = "SELECT Location from Locations ORDER BY [Location] ASC"; 
        $locationresult = sqlsrv_query($conn,$locationquery);
    ...
    <?php
		...
		{   
            ...
            $userArray = array($forename,$surname,$location,$date,$userID);            
			$query = "INSERT INTO userDetails ([Forename],[Surname],[Location],[Date],[userID]) VALUES (?,?,?,?,?)";
