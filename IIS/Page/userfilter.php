...
<div class="content">
	<h2 style="font-size:40px">Filter user</h2>
    <div id="MyClockDisplay" class="clock"></div>
  	<form method="POST" action="">
    <p>Select user from list to display below</p>  
    <?php
        require "Config.php";
        ...
        while ($row = sqlsrv_fetch_array($userresult)) {
            echo "<option 'value='".$row['Forename'].' '.$row['Surname'].' '.$row['userID']."'>".$row['Forename'].' '.$row['Surname'].' '.$row['userID']."</option>";
        }
        echo "</select>";
    ?>  
...
<table id="users">
	...
	<?php  
    	...
    	if($_POST['display_btn'])
    	{
			...
			$userlocationsquery = "SELECT  *
									FROM    userDetails
									WHERE   createdDate >= DATEADD(hour,-24 , GETDATE()) AND userID='$userID'
									ORDER BY entryID DESC";
			$userIDresult = sqlsrv_query($conn,$userlocationsquery);

			while($rows = sqlsrv_fetch_array($userIDresult))
        	{
            echo '<tr>
